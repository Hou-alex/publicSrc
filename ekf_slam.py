...
def motion_model(x, u):
    #motion_model有两个作用
    #如果输入是真实轨迹和有噪声控制，则输出下一个真实轨迹
    #如果输入是ground truth 和无噪声的控制，输出下一个ground truth
...
def main():
    ...
    xEst = np.zeros((STATE_SIZE, 1))#估计
    xTrue = np.zeros((STATE_SIZE, 1))#真值
    PEst = np.eye(STATE_SIZE)

    xDR = np.zeros((STATE_SIZE, 1))  # Dead reckoning 实际路线
    ...
    while SIM_TIME >= time:
        ...
        #xTrue ground truth轨迹
        #z 路标 [range  bearing]^T
        #xDR 真实轨迹
        # u 控制
        #RFID 真实路标
